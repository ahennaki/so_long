...
/*   Updated: 2023/02/26 20:07:48 by aennaki          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	place_player(t_data *game, int h, int w)
{
	if (game->dr == 0)
		mlx_put_image_to_window(game->mlxptr,
			game->winptr, game->pl, w * 50, h * 50);
	if (game->dr == 13)
		mlx_put_image_to_window(game->mlxptr,
			game->winptr, game->pb, w * 50, h * 50);
	if (game->dr == 2)
		mlx_put_image_to_window(game->mlxptr,
			game->winptr, game->pr, w * 50, h * 50);
	if (game->dr == 1)
		mlx_put_image_to_window(game->mlxptr,
			game->winptr, game->pf, w * 50, h * 50);
	game->y = h;
	game->x = w;
}

void	place_collectable(t_data *game, int h, int w)
{
	mlx_put_image_to_window(game->mlxptr,
		game->winptr, game->c, w * 50, h * 50);
	game->clt++;
}
